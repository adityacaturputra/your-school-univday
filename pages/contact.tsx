...
const formatIndoNumber = (contacts: Contact[]): Contact[] => {
  ...
  contacts.forEach((contact) => {
    const isFirstZeroEightNumber =
      contact.contact[0] === '0' && contact.contact[1] === '8';
    if (isFirstZeroEightNumber) {
      const newFormattedContact = `62${contact.contact
        .split('')
        .slice(1, contact.contact.length)
        .join('')}`;
...
const ContactPage: NextPage = () => {
    ...
    const fetchContact = async () => {
      try {
        ...
        const isPassed12Hour =
          new Date().getTime() >
          new Date(contactsFromLocalStorage?.updatedAt).getTime() +
            3600000 * 12;
        if (!contactsFromLocalStorage.data || isPassed12Hour) {
          const fetchedContact = await fetch(
            `${process.env.NEXT_PUBLIC_API_URI}/api/v1/contact`,
          );
          ...
          const localStorageContacts = {
            data: newFormattedContacts,
            updatedAt: new Date(),
          };
          localStorage.setItem(
            'contacts',
            JSON.stringify(localStorageContacts),
          );
  ...
  return (
    <>
      ...
      <ScrollableBox>
        <Fade>
          <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold m-8 ml-4 px-6 py-2 text-gray-700 border-b-2 border-gray-700 bg-gray-50 rounded">
            Contact Person
          </h1>
        </Fade>
