...
const Explore: NextPage<Props> = (props) => {
  ...
  const getData = async () => {
    try {
      setloading(true);
      const universityFromLocalStorage = JSON.parse(
        localStorage.getItem('university') || '{}',
      );
      setuniversity(universityFromLocalStorage.data);
      const isPassed12Hour =
        new Date().getTime() >
        new Date(universityFromLocalStorage?.updatedAt).getTime() +
          12 * 3600000;
      if (!universityFromLocalStorage.data || isPassed12Hour) {
        const fetchedUniversity = await fetch(
          `${process.env.NEXT_PUBLIC_API_URI}/api/v1/university`,
        );
        const dataUniversity = await fetchedUniversity.json();
        const university: University[] = dataUniversity.university.sort(
          (a: University, b: University) => b.priorityLevel - a.priorityLevel,
        );
        const localStorageUniversity = {
          data: university,
          updatedAt: new Date(),
        };
        localStorage.setItem(
          'university',
          JSON.stringify(localStorageUniversity),
        );
        setuniversity(university);
        setlastFetched(localStorageUniversity.updatedAt.toLocaleString());
      } else {
        setlastFetched(
          new Date(universityFromLocalStorage.updatedAt).toLocaleString(),
        );
        setuniversity(universityFromLocalStorage.data);
      }
      setloading(false);
    ...
    <>
      ...
      {university?.length > 0 && (
        <>
          <Logos
            data={university}
            currentIndex={DetailUniversityIndex}
            onClick={handleDetailUniversityIndex}
          />
          <Fade>
          ...
          <Heading
            ...
            title={university[DetailUniversityIndex].name}
          />
        </>
      )}
      {errorMessage && (
