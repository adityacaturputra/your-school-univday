...
const Contents: React.FC<Props> = ({
  ...
  const handleRefreshUnivContent = (id: string) => async () => {
    ...
    try {
      const fetchedUniversity = await fetch(
        `${process.env.NEXT_PUBLIC_API_URI}/api/v1/university/${id}`,
      );
      const { university } = await fetchedUniversity.json();
      const universityLocalStorage = JSON.parse(
        localStorage.getItem('university') || '{}',
      );
      universityLocalStorage.data[currentIndex] = {
        ...university,
        lastFetched: new Date().toLocaleString(),
      };
      localStorage.setItem(
        'university',
        JSON.stringify(universityLocalStorage),
      );
  ...
  data[currentIndex].contentId = data[currentIndex].contentId.sort(
      ...
      <div className="h-[105vh] mt-[16vh] relative container mx-auto">
        <div className="scrollup max-h-[100%] overflow-y-auto overflow-x-hidden pt-[16vh] pb-[20vh]">
            ...
            {!isLoading ? (
              <>
                <p className="text-gray-400 text-xs sm:text-sm">
                  ...
                  {data[currentIndex]?.lastFetched || globalLastFetched}
                </p>
                <img
                  onClick={handleRefreshUnivContent(data[currentIndex]._id)}
          ...
          {data[currentIndex].contentId.map((konten: Content) => (
            <div
              key={konten._id}
              className=" overflow-auto bg-gradient-to-r from-white to-gray-50 my-4 mb-16 mx-3 p-3 md:p-8 lg:p-16 rounded scrollup"
            >
